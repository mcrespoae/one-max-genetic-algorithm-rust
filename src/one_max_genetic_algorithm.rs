...
pub fn select_parent(population: &Vec<Vec<u8>>, fitness_values: &Vec<f64>, mode: &str) -> Vec<u8> {
    if mode.to_lowercase() == "roulette" || !["roulette", "tournament"].contains(&mode.to_lowercase().as_str()) {
        return select_parent_roulette(population, fitness_values);
    } else {
        let tournament_size: usize = thread_rng().gen_range(((*population).len() as f64 * 0.6) as usize..=((*population).len() as f64 * 0.8) as usize);
...
pub fn genetic_algorithm(population_size: usize, genome_length: usize,
    ...
    for generation in 0..max_generations {
        ...
        let prev_generation_fitness = get_generation_fitness(&fitness_values, population_size);
        ...
        if (best_gen_fitness - target_fitness).abs() < f64::EPSILON && generation_fitness >= target_generation_fitness {
            if verbose {
