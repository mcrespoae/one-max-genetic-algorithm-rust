...
mod utils;
use utils::{generate_equally_spaced_values, distribute_run_times};
...
pub fn process_genetic_algorithm(mutation_rate_values: &[f64], crossover_rate_values: &[f64]){
    ...
    println!("--------------------------------------------------");
    println!("Best Mutation Rate: {best_mutation_rate}\nBest Crossover Rate: {best_crossover_rate}");
    best_result.print();

}

fn main() {
    let mutation_rate_values: Vec<f64> = generate_equally_spaced_values(MUTATION_RATE_MIN, MUTATION_RATE_MAX, 8, true);
    let crossover_rate_values: Vec<f64> = generate_equally_spaced_values(CROSSOVER_RATE_MIN, CROSSOVER_RATE_MAX, 5, false);


    println!("Running {} times the one max problem with genetic algorithms for:
    Generations:           {}
    Population Size:       {}
    Genome Length:         {}
